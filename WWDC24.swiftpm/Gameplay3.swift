        ...
        if let index = source.firstIndex(of: dice) {
            ...
            if removedDice.imageName.contains("blue") {
                if finalNumeratorArray.contains(value) {
                    ...
                    if dice.value < 9 {
                        ...
                        textsWithColors[textsWithColors.count - 1] = [Text("\(Text("The number").foregroundColor(.white)) \(Text("\(dice.value)").foregroundColor(Color(hex: "0094FF"))) \(Text("it's not possible to take in a").foregroundColor(.white)) \(Text("8-sided dice!").foregroundColor(Color(hex: "F03131")))")]
                        auxIndex = 2
                        face = "archersad"
                        waiting = true
                        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            // Se for vermelho, verifique se o valor existe no final do denominador
                ...
                if finalDenominatorArray.contains(value) {
                    ...
                    textsWithColors[textsWithColors.count - 1] = [Text("\(Text("The number").foregroundColor(.white)) \(Text("\(dice.value)").foregroundColor(Color(hex: "F03131"))) \(Text("it's not a possibility in a").foregroundColor(.white)) \(Text("8-sided dice").foregroundColor(Color(hex: "F03131")))")]
                    auxIndex = 2
                    face = "archersad"
                    waiting = true
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
